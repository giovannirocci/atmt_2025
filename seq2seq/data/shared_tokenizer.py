...
class BPETokenizer:
    ...
    **Only used for training** the tokenizer, inference uses the SentencePieceProcessor directly."""
    def __init__(self, src_language: str, tg_language: str, vocab_size: int,
                     eos='</s>', bos='<s>', pad='<pad>', unk='<unk>'):
        self.SRC_LANG = src_language
        self.TG_LANG = tg_language
    ...
    def train_tokenizer(self, training_data: str, model_dir: str):
        ...
        model_name = f'{self.SRC_LANG}-{self.TG_LANG}-bpe-{self.vocab_size}'
    ...
    @classmethod
    def load_from_model_only(cls, model_path: str, src_language: str, tg_language: str):
        """Loads the SentencePiece tokenizer from the specified directory."""
        tokenizer = spm.SentencePieceProcessor()
        tokenizer.load(model_path)
        vocab_size = tokenizer.get_piece_size()
        instance = cls(src_language=src_language, tg_language=tg_language, vocab_size=vocab_size)
    ...
    def save_vocab(self, model_dir):
        ...
        **NOTE**: this is only for inspection by the user, the Dictionary class uses the model files to load the actual vocabulary."""
        ...
        vocab_file = os.path.join(os.path.normpath(model_dir), f'{self.SRC_LANG}-{self.TG_LANG}-bpe-{self.vocab_size}.vocab')
